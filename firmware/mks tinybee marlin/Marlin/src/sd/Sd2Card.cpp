...
bool DiskIODriver_SPI_SD::init(const uint8_t sckRateID, const pin_t chipSelectPin) {
  ...
  while ((status_ = cardCommand(CMD0, 0)) != R1_IDLE_STATE) {
    if (ELAPSED(millis(), init_timeout)) {
      SERIAL_ERROR_MSG("SD TIME OUT, SD_CARD_ERROR_CMD0\n");
  ...
  for (;;) {
    ...
    if (ELAPSED(millis(), init_timeout)) {
      SERIAL_ERROR_MSG("SD_CARD_ERROR_CMD8\n");
  ...
  while ((status_ = cardAcmd(ACMD41, arg)) != R1_READY_STATE) {
    // Check for timeout
    if (ELAPSED(millis(), init_timeout)) {
      SERIAL_ERROR_MSG("SD_CARD_ERROR_ACMD41\n");
  ...
  if (type() == SD_CARD_TYPE_SD2) {
    if (cardCommand(CMD58, 0)) {
      SERIAL_ERROR_MSG("SD_CARD_ERROR_CMD58\n");
